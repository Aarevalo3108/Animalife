...
const ShoppingCart = ({ className, enableRender }) => {
    ...
    <div className={"p-4 grid gap-[0.5rem] w-52 max-h-96 overflow-y-auto border-2 border-[#aa7e5d] border-dashed absolute transform top-20 right-2 bg-[url('/svg/notebook.svg')] rounded-lg transition-all duration-500 ease-in-out " + className}>
      ...
      <div className="grid gap-2 min-h-16">
        {data.length && data.length > 0 ? (
          ...
          loader ? <Loading /> : <p className="place-self-center">{enableRender ? "Cart is empty" : " "}</p>
        )}
      </div>
      <Link className="place-self-center px-2 py-1 rounded-lg border-2 border-[#aa7e5d] bg-[#f2e0c2] hover:bg-[#aa7e5d] hover:text-[#f2e0c2] transition duration-150" to="/cart">View Details</Link>
    </div>
  );
};
