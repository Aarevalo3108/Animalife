...
const AdminStats = () => {
  ...
  */
  ...
  const getTopSelling = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${url.backend}/product?sort=-sales&limit=5`, {
        headers: {
          "Authorization": `Bearer ${auth.getAccessToken()}`,
        },
      });
    ...
    <div className="grid justify-items-center gap-4 px-4 py-8">
      ...
      <ul id="svg" className="grid grid-cols-5 sticky top-16 gap-2 py-4">
        <li className="truncate">
          <a href="#Lastmonth" className="flex items-center justify-center gap-2 bg-n3 text-n1 p-2 hover:bg-n2 hover:text-n5 transition duration-150 rounded-lg">
            <span className="hidden md:text-sm md:block lg:text-lg">Last Month Sales</span>
            <img className="h-6 w-6 md:hidden lg:h-4 lg:w-4" src="/svg/orders.svg" alt="Month" />
          </a>
        </li>
        <li className="truncate">
          <a href="#TopSelling" className="flex items-center justify-center gap-2 bg-n3 text-n1 p-2 hover:bg-n2 hover:text-n5 transition duration-150 rounded-lg">
            <span className="hidden md:text-sm md:block lg:text-lg">Top Selling</span>
            <img className="h-6 w-6 md:hidden lg:h-4 lg:w-4" src="/svg/items.svg" alt="Sold" />
          </a>
        </li>
        <li className="truncate">
          <a href="#TopOrders" className="flex items-center justify-center gap-2 bg-n3 text-n1 p-2 hover:bg-n2 hover:text-n5 transition duration-150 rounded-lg">
            <span className="hidden md:text-sm md:block lg:text-lg">Top Orders</span>
            <img className="h-6 w-6 md:hidden lg:h-4 lg:w-4" src="/svg/purchases.svg" alt="Sold" />
          </a>
        </li>
        <li className="truncate">
          <a href="#TopBuyers" className="flex items-center justify-center gap-2 bg-n3 text-n1 p-2 hover:bg-n2 hover:text-n5 transition duration-150 rounded-lg">
            <span className="hidden md:text-sm md:block lg:text-lg">Top Buyers</span>
            <img className="h-6 w-6 md:hidden lg:h-4 lg:w-4" src="/svg/users.svg" alt="Sold" />
          </a>
        </li>
        <li className="truncate">
          <a href="#Totals" className="flex items-center justify-center gap-2 bg-n3 text-n1 p-2 hover:bg-n2 hover:text-n5 transition duration-150 rounded-lg">
            <span className="hidden md:text-sm md:block lg:text-lg">Totals</span>
            <img className="h-6 w-6 md:hidden lg:h-4 lg:w-4" src="/svg/total.svg" alt="Sold" />
          </a>
        </li>
      </ul>
      <div id="Lastmonth" className="grid pt-8 justify-items-center items-center min-h-[30vh]">
        ...
        <div className="grid grid-cols-2 justify-items-center items-center gap-8">
          <h2 className="text-2xl col-span-2"> Last Moth Sales: ${sales}</h2>
          <h3 className="text-xl">Total Items Sold: {sold}</h3>
      ...
      <div id="TopSelling" className="grid pt-32 justify-items-center items-center min-h-[60vh]">
        ...
        {TopSelling.length > 0 &&
          <div className="grid justify-items-center items-center gap-8 py-8">
            <h2 className="text-2xl">Top 5 Selling</h2>
              ...
              {TopSelling.map((item) => {
                return (
                  <div key={item._id} className="flex bg-n1 p-4 rounded-lg shadow-lg flex-col justify-center items-center gap-2">
                    <img className="h-56 w-40 rounded-lg object-cover" src={url.backend + "/" + item.images[0]} alt="" />
                    <h3 className="text-xl w-56 text-center">{item.name}</h3>
      ...
      <div id="TopOrders" className="grid pt-32 justify-items-center items-center min-h-[60vh]">
        ...
        {TopOrders.length > 0 &&
          <div className="grid justify-items-center items-center gap-8 py-8">
            <h2 className="text-2xl">Top 5 Orders</h2>
            <div className="grid md:grid-cols-3 justify-items-center items-center gap-8">
              {TopOrders.map((item) => {
                return (
      ...
      <div id="TopBuyers" className="grid pt-32 justify-items-center items-center min-h-[60vh]">
        ...
        {topBuyers.length > 0 &&
          <div className="grid justify-items-center items-center gap-8 py-8">
            <h2 className="text-2xl">Top 5 Buyers</h2>
            <div className="grid md:grid-cols-3 justify-items-center items-center gap-8">
              {topBuyers.map((item) => {
                return (
      ...
      <div id="Totals" className="grid pt-32 justify-items-center items-center min-h-[60vh]">
        ...
        {topBuyers.length > 0 &&
          <div className="grid justify-items-center items-center gap-4 py-8">
            ...
            <h3 className="text-xl">Total Users: {totalUsers}</h3>
            <h3 className="text-xl">Total Orders: {totalOrders}</h3>
            <h3 className="text-xl">Total Products: {totalProducts}</h3>
          </div>}
      </div>
    </div>
  )
