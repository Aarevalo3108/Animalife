...
const Nav = ({background}) => {
  ...
  return (
    <nav className={"bg-repeat flex items-center justify-end md:justify-between gap-4 p-4 sticky top-0 z-50 " + background}>
      <Link to="/" className="flex gap-2 items-center">
        <img className="w-10 h-10 hover:scale-110 transition-transform duration-300" src="/svg/cat.svg" alt="" />
        <span className="hidden md:flex font-bold text-xl md:text-3xl hover:scale-110 transition-transform duration-300">Animalife</span>
      </Link>
      <div className="flex gap-4 items-center justify-center">
        <Link className={"hidden md:flex text-2xl hover:text-n1 transition-colors duration-300"} to="/" onClick={() => setShowPanel(false)}>Home</Link>
        <Link className={"hidden md:flex text-2xl hover:text-n1 transition-colors duration-300"} to="/shop" onClick={() => setShowPanel(false)}>Shop</Link>
      ...
      <button className="md:hidden hover:scale-110 transition-transform duration-300 absolute left-4 z-10" onClick={() => setShowPanel(!showPanel)}>
